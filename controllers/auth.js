...
const { ROLE } = require("../utils/enum"); // import
const { TYPE } = require("../utils/enum"); // import
...
module.exports = {
  ...
  register: async (req, res, next) => {
    try {
      ...
      if (newUser) {
        htmlEmail = await util.email.getHtml("welcome-mail.ejs", {
          name: newUser.name,
        });
        await util.email.sendEmail(newUser.email, "Welcome!", htmlEmail);
      }
